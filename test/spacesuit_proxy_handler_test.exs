defmodule SpacesuitProxyHandlerTest do
  ...
  test "converting headers to Hackney format and adding proxy info" do
    ...
    original_url = [
      [["http", 58], "//", "localhost", [58, "8080"]],
      "/v1/people/123/things",
      "",
      ""
    ]
    ...
    assert {"accept-language", "en-US,en;q=0.5"} = List.keyfind(processed, "accept-language", 0)

    assert {"x-forwarded-url", ^original_url} = List.keyfind(processed, "x-forwarded-url", 0)

    assert {"x-forwarded-host", "localhost"} = List.keyfind(processed, "x-forwarded-host", 0)
  ...
  test "building the upstream url when destination is set and no bindings exist" do
    ...
    url =
      Spacesuit.ProxyHandler.build_upstream_url(
        req,
  ...
  test "request_upstream passes the body when there is one" do
    result =
      Spacesuit.ProxyHandler.request_upstream(
        "get",
        "http://example.com",
        [{"Content-Type", "html"}],
        %{has_body: true, body: "test body"}
      )
